...
function getHttpsConfig(env) {
  ...
  const certPath = env.VITE_DEV_HTTPS_CERT || path.resolve(__dirname, 'certs/dev-cert.pem');
  const keyPath = env.VITE_DEV_HTTPS_KEY || path.resolve(__dirname, 'certs/dev-key.pem');
  try {
