...
import { createMoMoOrder, verifyIpnSignature, isMomoConfigured } from '../utils/momo.js';
...
export const Plans = {
  ...
  '6m': { months: 6, discount: 0.10 },
  '12m': { months: 12, discount: 0.20 }
};
...
export const getPlans = asyncHandler(async (req, res) => {
  const map = Object.fromEntries(
    Object.keys(Plans).map(k => [k, { months: Plans[k].months, amount: calcPlanAmount(k) }])
  );
...
export const createMomoPayment = asyncHandler(async (req, res) => {
  ...
  if (!isMomoConfigured()) {
    return res.status(400).json({ success: false, message: 'MoMo chưa được cấu hình đầy đủ. Vui lòng kiểm tra các biến môi trường MOMO_* trong .env' });
  }
  ...
  const extraDataObj = { paymentId: String(pay._id), userId: String(userId), plan };
  ...
  try {
    ...
    pay.status = 'failed';
    pay.meta = { orderId, reason: 'Invalid MOMO_REDIRECT_URL or MOMO_IPN_URL format' };
    await pay.save();
    return res.status(400).json({ success: false, message: 'MOMO_REDIRECT_URL hoặc MOMO_IPN_URL không hợp lệ (không phải URL đầy đủ). Vui lòng cập nhật .env' });
  }

  try {
    const momoResp = await createMoMoOrder({ amount, orderId, orderInfo, extraDataObj });
    if (momoResp?.resultCode === 0 && momoResp?.payUrl) {
      pay.meta = { orderId, requestId: momoResp.requestId, deeplink: momoResp.deeplink };
      await pay.save();
      return res.json({ success: true, data: { payUrl: momoResp.payUrl, orderId, paymentId: pay._id } });
    }
    // Non-zero resultCode
    pay.status = 'failed';
    pay.meta = { orderId, reason: momoResp?.message || 'Create order failed', momo: momoResp };
    await pay.save();
    return res.status(400).json({ success: false, message: momoResp?.message || 'Create order failed', data: momoResp });
  } catch (e) {
    // Axios/non-2xx or signature mismatch etc.
    pay.status = 'failed';
    pay.meta = { orderId, reason: e.message || 'Create order exception', momo: e.momoResponse };
    await pay.save();
    const status = e.statusCode && e.statusCode >= 400 && e.statusCode < 500 ? e.statusCode : 400;
    return res.status(status).json({ success: false, message: e.message || 'Create order failed', data: e.momoResponse || null });
  }
});
...
export const momoIpnHandler = asyncHandler(async (req, res) => {
  ...
  if (Number(amount) !== Number(payment.amount)) {
    ...
    payment.meta = { ...(payment.meta || {}), reason: 'Amount mismatch', reportedAmount: Number(amount) };
    await payment.save();
  ...
  if (Number(resultCode) === 0) {
    ...
    if (user) {
      const now = new Date();
      const base = user.proExpiry && new Date(user.proExpiry) > now ? new Date(user.proExpiry) : now;
...
export const momoReturnRedirect = asyncHandler(async (req, res) => {
  ...
  if (verifyIpnSignature(payload)) {
    try {
      ...
      if (!paymentId && orderId?.includes('_')) paymentId = String(orderId).split('_')[1];
      if (paymentId) {
        const payment = await Payment.findById(paymentId);
        if (payment) {
          if (payment.status !== 'paid') {
            if (Number(amount) !== Number(payment.amount)) {
              payment.status = 'failed';
              payment.meta = { ...(payment.meta || {}), reason: 'Amount mismatch (return)', reportedAmount: Number(amount) };
              await payment.save();
  ...
  const clientBase = process.env.WEB_URL || process.env.APP_WEB_URL || process.env.CLIENT_URL || 'http://localhost:5173';
...
export const momoReturnConfirm = asyncHandler(async (req, res) => {
  ...
  if (!verifyIpnSignature(payload)) {
    return res.status(400).json({ success: false, message: 'Invalid signature (confirm)' });
  }
  const { resultCode, amount, orderId, extraData } = payload;
  let paymentId;
  try {
  ...
  if (Number(amount) !== Number(payment.amount)) {
    payment.status = 'failed';
    payment.meta = { ...(payment.meta || {}), reason: 'Amount mismatch (confirm)', reportedAmount: Number(amount) };
    await payment.save();
    return res.status(400).json({ success: false, message: 'Amount mismatch' });
  }
